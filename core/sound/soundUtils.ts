...
export const getProfessionalVoices = (): SpeechSynthesisVoice[] => {
    if (!('speechSynthesis' in window)) return [];
    
    const voices = window.speechSynthesis.getVoices();
    
    // Filter for English voices that usually sound professional
    return voices.filter(v => 
        (v.lang.includes('en-US') || v.lang.includes('en-GB')) &&
        !v.name.toLowerCase().includes('kb') // Remove some low quality robotic ones if present
    ).sort((a, b) => {
...
export const speakText = (text: string, voiceURI?: string) => {
  ...
  // Fallback if specific voice not found or not provided
  if (!preferredVoice) {
